...
#include <igl/paper_cost_and_new_vertex.h>
	...
	namespace opengl
	{
		namespace glfw
		{
			...
			//    std::vector<ViewerPlugin*> plugins;
				...
				std::vector<Eigen::MatrixXd *> OVs{10}; // Backup of original vertices of the current mesh (#V x 3)
				std::vector <Eigen::MatrixXi *> OFs{10}; // Backup of original faces of the mesh (#F x 3)

				std::vector<Eigen::VectorXi *> EMAPs{10}; // Maps faces to edges (#F x 3)
				std::vector<Eigen::MatrixXi *> Es{10}; // Edges of the mesh (#E x 2)
				std::vector<Eigen::MatrixXi *> EFs{10}; // Maps edges to faces (#E x 2)
				std::vector<Eigen::MatrixXi *> EIs{10}; // Maps edges to their index in the relevant entry at EMAP(e) (#E x 2)

				typedef std::set<std::pair<double, int>> Priority_queue; // Ordered set of pair<cost, edge index>
				std::vector<Priority_queue *> Qs{10}; // A priority queue for the Q matrices of the mesh
				std::vector<std::vector<Priority_queue::iterator>> Qits{10};
				std::vector<Eigen::MatrixXd *> Cs{10}; // New vertices after collapse
				std::vector<int> nums_collapsed; // Number of collapsed edges for the mesh

			public:
				EIGEN_MAKE_ALIGNED_OPERATOR_NEW
				// animation in 3D assignment func
				void init_curr_data_structs();
				void reset();
				void pre_draw();
				void print_data_structs();
				inline void set_curr_reset_point()
				{
					OVs[selected_data_index] = new Eigen::MatrixXd(data().V);
					OFs[selected_data_index] = new Eigen::MatrixXi(data().F);
				}
			};
