...
namespace igl
{
                                      ...
                                      Eigen::PlainObjectBase<DerivedEC> &EC)
  {
    ...
    helper = [&helper,
              &P, &N, &A, &expansion_order, &point_indices, &CH, &EC, &R, &CM](const int index) -> void
    {
                                      ...
                                      Eigen::PlainObjectBase<DerivedWN> &WN)
  {
    ...
    {
      real_wn wn = (loc(0) * anorm(0) + loc(1) * anorm(1) + loc(2) * anorm(2)) / (4.0 * igl::PI * std::pow(loc.norm(), 3));
      if (std::isnan(wn))
      {
        return 0.5;
      }
    ...
    helper = [&helper,
              ...
              &point_indices, &CH,
              &CM, &R, &EC, &beta,
              &direct_eval, &expansion_eval](const RowVec query, const std::vector<int> near_indices) -> real_wn
    {
