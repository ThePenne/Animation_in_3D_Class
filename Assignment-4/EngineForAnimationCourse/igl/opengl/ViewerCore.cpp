...
IGL_INLINE void igl::opengl::ViewerCore::draw_cubemap(
  const Eigen::Matrix4f &worldMat,
  ViewerData& data,
  bool update_matrices)
{
  ...
  glBindTexture(GL_TEXTURE_CUBE_MAP, data.cubemap_texture);
  ...
  GLint proji  = glGetUniformLocation(data.meshgl.shader_mesh,"proj");
  GLint normi  = glGetUniformLocation(data.meshgl.shader_mesh,"normal_matrix");
  viewi = glGetUniformLocation(data.meshgl.shader_cubemap, "view");
  proji = glGetUniformLocation(data.meshgl.shader_cubemap, "proj");
