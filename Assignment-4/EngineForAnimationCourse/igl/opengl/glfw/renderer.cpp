...
IGL_INLINE void Renderer::draw(GLFWwindow *window)
{
	...
	if (menu)
	{
		if (scn->game_over)
		{
			menu->menu_mode = igl::opengl::glfw::imgui::ImGuiMenu::game_over;
			scn->game_over = false;
			scn->isActive = false;
		}
		else if (scn->level_finished)
		{
			mciSendString("close level_finish", NULL, 0, NULL);
			mciSendString("open \"OST\\win_level_tone.mp3\" type mpegvideo alias level_finish", NULL, 0, NULL);
			mciSendString("play level_finish", NULL, 0, NULL);
			menu->menu_mode = igl::opengl::glfw::imgui::ImGuiMenu::level_finished;
			scn->level_finished = false;
			scn->isActive = false;
		}
		menu->pre_draw();
	...
	for (auto &core : core_list)
	{
		...
		for (auto &mesh : scn->data_list)
		{
			...
			{ // for kinematic chain change scn->MakeTrans to parent matrix
				...
				if (indx == scn->cubemap)
					core.draw_cubemap(scn->MakeTransScale() * scn->CalcParentsTrans(indx).cast<float>(), mesh);
				else
...
IGL_INLINE void Renderer::init(igl::opengl::glfw::Viewer *viewer, int coresNum, igl::opengl::glfw::imgui::ImGuiMenu *_menu)
{
	...
	core().viewport = Eigen::Vector4f(0, 0, width, height);
	view = core_list[0].id;
...
double Renderer::Picking(double newx, double newy)
{
	...
	view = view * (core().trackball_angle * Eigen::Scaling(core().camera_zoom * core().camera_base_zoom) * Eigen::Translation3f(core().camera_translation + core().camera_base_translation)).matrix() * scn->MakeTransScale() * scn->CalcParentsTrans(scn->selected_data_index).cast<float>() * scn->data().MakeTransScale();
...
IGL_INLINE void Renderer::post_resize(GLFWwindow *window, int w, int h)
{
	...
	else
	{
		...
		core(view).viewport = Eigen::Vector4f(0, 0, w / 4, h);
	}
...
// IGL_INLINE void Viewer::select_hoverovered_core()
//{
