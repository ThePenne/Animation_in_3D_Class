...
static void glfw_mouse_press(GLFWwindow *window, int button, int action, int modifier)
{
	...
	// if (action == GLFW_PRESS && !rndr->in_game())
	// {
	// 	double x2, y2;
	...
	// 	scn->data().set_colors(Eigen::RowVector3d(181./255., 131./255., 65./255.));
	// 	if (lastIndx != savedIndx)
	// 		scn->data_list[lastIndx].set_colors(Eigen::RowVector3d(181./255., 131./255., 65./255.));
...
static void glfw_key_callback(GLFWwindow *window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
	...
	else if (rndr->in_game() && (action == GLFW_PRESS || action == GLFW_REPEAT))
		switch (key)
		{
		case 'A':
		case 'a':
		...
		case 'r':
		{
			// rndr->core().camera_eye << -3,0,0;
			// rndr->core().camera_up << 0,0,1;
			break;
		}
		case '[':
		case ']':
		...
		case GLFW_KEY_C:
			...
			break;
		case GLFW_KEY_UP:
			scn->SetDirection(SandBox::ARROW_UP);
			scn->rotate_camera(1);
			break;
		case GLFW_KEY_DOWN:
			scn->SetDirection(SandBox::ARROW_DOWN);
			scn->rotate_camera(-1);
			break;
		case GLFW_KEY_LEFT:
			scn->SetDirection(SandBox::ARROW_LEFT);
			break;
		case GLFW_KEY_RIGHT:
			scn->SetDirection(SandBox::ARROW_RIGHT);
			break;
		case 'P':
		case 'p':
			rndr->pause_game();
